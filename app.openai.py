...
def get_embedding(text, model="text-embedding-ada-002"):
    """Generate embeddings using OpenAI's new API format"""
...
parsed_resume = {
    ...
    "embedding":  get_embedding(resume_text) # get_embedding(resume_text)
}
...
def search_candidates(job_description, top_k=1):
    """Find top-k candidates from PostgreSQL using FAISS"""
    job_vector = get_embedding(job_description)  # Generate embedding
    job_vector = np.array(job_vector, dtype=np.float32)  # Convert to float32
    job_vector = job_vector.reshape(1, -1)  # Ensure shape (1, 1536)

    print(f"FAISS index dimension: {index.d}")
    print(f"Job vector shape: {job_vector.shape}")  # Debugging
...
# Example Query
job_description = "seach a candidate with experiece in fintech domain as manager having education as B.Tech as degree"
matches = search_candidates(job_description, top_k=1)
